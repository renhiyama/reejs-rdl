...
export async function run(c, msg) {
  ...
  console.log(msg.data.resolved.members[0])
  if (type == "bot") {
    let bots = await db.query(`SELECT * FROM bots WHERE botID = $id`, { id });
    let bot = bots[0].result[0];
    if (!bot) {
      return c.json({ type: 4, data: { content: "This bot is not on RDL. Please ask the owner to add it." } });
    }
    return c.json({
      type: 4,
      data: {
    ...
    if (!user) {
      return c.json({ type: 4, data: { content: "User didn't create an account on RDL." } });
    }
    return c.json({
      type: 4,
      data: {
